...
int main(void)
{
    ...
    DMA dma1 (0) ;
		dma1.getTransferType (M2M);
    dma1.source (DMA_SRC,0,1);
		dma1.destination (DMA_DST,0,1);
...
/*test m2P, send the memory data to UART;*/
 ...
 /* Initialize structure */
    LPC_UART0->FCR |= 1<<3 ; //enable UART DMA mode
		dma3.destination ((uint32_t)&(LPC_UART0->THR),0,0);
		dma3.source(DMA_SRC2,0,1);
		dma3.getTransferType(M2P);
		dma3.trigger(UART0_TX);
		dma3.attach_TC(led_switchoff);
		dma3.start(size2);
		dma3.wait();
